...
export const projects = [
  ...
  },
  {
    ...
    result: {
      fa: 'دیگر شمارش سالانه با کاغذ انجام نمی‌شود؛ هر دوره با موبایل ثبت می‌شود و مغایرت‌ها همان لحظه روی داشبورد مشخص است، نه در پایان سال.',
      en: 'The annual paper count is gone; each cycle is logged from a phone, and discrepancies show up on the dashboard as they happen instead of at year-end.',
      de: 'Die jährliche Papierzählung entfällt; jeder Zyklus wird per Handy erfasst, und Abweichungen erscheinen sofort im Dashboard statt erst zum Jahresende.',
    },
  ...
  },
  {
    ...
    tagline: {
      fa: 'وب‌سایت شرکتی یک تولیدکننده‌ی تجهیزات پزشکی، بازنویسی‌شده با React و Django به‌جای قالب آماده‌ی وردپرس.',
      en: 'The corporate site for a medical-equipment manufacturer, rebuilt in React and Django in place of a stock WordPress theme.',
      de: 'Die Firmenwebsite eines Medizintechnik-Herstellers, neu gebaut mit React und Django statt eines fertigen WordPress-Themes.',
    },
    overview: {
      fa: 'وب‌سایت شرکتی «نیکی‌پژوه» (تجهیزات پزشکی) دیگر روی یک قالب آماده‌ی وردپرس اجرا نمی‌شود؛ یک اپلیکیشن اختصاصی با React در فرانت و Django در بک‌اند جایگزین آن شد تا سرعت، مدیریت محتوا و مقیاس‌پذیری بهتر شود.',
      en: 'The corporate site for Niki Pajouh (medical equipment) no longer runs on a stock WordPress theme. In its place is a custom application — React on the frontend, Django on the backend — built for better speed, content management, and scalability.',
      de: 'Die Firmenwebsite von Niki Pajouh (Medizintechnik) läuft nicht mehr auf einem fertigen WordPress-Theme. An seine Stelle trat eine individuelle Anwendung — React im Frontend, Django im Backend — für bessere Geschwindigkeit, Content-Verwaltung und Skalierbarkeit.',
    },
    problem: {
    ...
    result: {
      fa: 'صفحات سریع‌تر بارگذاری می‌شوند و تیم داخلی شرکت محصولات و مقالات را بدون دخالت توسعه‌دهنده مدیریت می‌کند.',
      en: 'Pages load faster, and the in-house team manages products and articles without needing a developer.',
      de: 'Seiten laden schneller, und das interne Team verwaltet Produkte und Artikel ohne Entwickler.',
    },
  ...
  },
  {
    ...
    result: {
      fa: 'کاربر دیگر بین چند سایت جابه‌جا نمی‌شود؛ جست‌وجو و پیگیری استخدام همه در یک اپلیکیشن انجام می‌شود.',
      en: 'No more switching between separate sites — searching and tracking applications now happen in one app.',
      de: 'Kein Wechseln zwischen mehreren Seiten mehr — Suche und Bewerbungsverfolgung laufen jetzt in einer App.',
    },
  ...
  },
  {
    ...
    result: {
      fa: 'خط تولید دیگر منتظر تست و برچسب‌زنی دستی نمی‌ماند؛ هر سه کار خودکار انجام می‌شود، سریع‌تر و با خطای کمتر.',
      en: 'The line no longer waits on manual testing and labeling; all three run automatically now, faster and with fewer errors.',
      de: 'Die Linie wartet nicht mehr auf manuelles Testen und Etikettieren; alle drei Schritte laufen jetzt automatisch, schneller und mit weniger Fehlern.',
    },
    stack: ['Python'],
    links: {},
  },
